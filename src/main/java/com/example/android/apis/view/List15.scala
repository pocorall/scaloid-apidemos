 ...
 */
class List15 extends ListActivity {
  // Scaloid >>
  ...
  private class ModeCallback extends android.widget.AbsListView.MultiChoiceModeListener {
    ...
    def onActionItemClicked(mode: ActionMode, item: MenuItem): Boolean = {
      item.getItemId match {
        case R.id.share =>
          Toast.makeText(List15.this, "Shared " + getListView.getCheckedItemCount + " items", Toast.LENGTH_SHORT).show
          mode.finish
        case _ =>
          Toast.makeText(List15.this, "Clicked " + item.getTitle, Toast.LENGTH_SHORT).show
      }
      return true
    }
    ...
    private def setSubtitle(mode: ActionMode) {
