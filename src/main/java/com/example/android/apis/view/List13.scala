...
class List13 extends ListActivity with AbsListView.OnScrollListener {
  ...
  def onScrollStateChanged(view: AbsListView, scrollState: Int) {
    scrollState match {
      case OnScrollListener.SCROLL_STATE_IDLE =>
        mBusy = false
        val first: Int = view.getFirstVisiblePosition
        val count: Int = view.getChildCount

        var i: Int = 0
        while (i < count) {
          {
            val t: TextView = view.getChildAt(i).asInstanceOf[TextView]
            if (t.getTag != null) {
  ...
  private class SlowAdapter extends BaseAdapter {
    ...
    def this(context: Context) {
      this()
      mInflater = context.getSystemService(Context.LAYOUT_INFLATER_SERVICE).asInstanceOf[LayoutInflater]
    }
    ...
    def getView(position: Int, convertView: View, parent: ViewGroup): View = {
      ...
      if (convertView == null) {
        text = mInflater.inflate(android.R.layout.simple_list_item_1, parent, false).asInstanceOf[TextView]
      }
      else {
        text = convertView.asInstanceOf[TextView]
      }
      if (!mBusy) {
