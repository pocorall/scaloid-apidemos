 ...
 */
class HoverInterceptorView(context: Context, attrs: AttributeSet) extends LinearLayout(context, attrs) {
  private var mInterceptHover: Boolean = false

  override def onInterceptHoverEvent(event: MotionEvent): Boolean = {
    if (mInterceptHover) {
      return true
    }
  ...
  override def onHoverEvent(event: MotionEvent): Boolean = {
    val textView: TextView = findViewById(R.id.intercept_message).asInstanceOf[TextView]
  ...
  def setInterceptHover(intercept: Boolean) {
