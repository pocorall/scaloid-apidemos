...
class TableLayout7 extends Activity {
  protected override def onCreate(savedInstanceState: Bundle) {
    ...
    val table: TableLayout = findViewById(R.id.menu).asInstanceOf[TableLayout]
    var button: Button = findViewById(R.id.toggle1).asInstanceOf[Button]
  ...
  private def appendRow(table: TableLayout) {
    val row: TableRow = new TableRow(this)
    val label: TextView = new TextView(this)
    label.setText(R.string.table_layout_7_quit)
    label.setPadding(3, 3, 3, 3)
    val shortcut: TextView = new TextView(this)
  ...
  private var mShortcutsCollapsed: Boolean = false
  private var mCheckmarksCollapsed: Boolean = false
}